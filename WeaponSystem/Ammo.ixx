...
export [[nodiscard]] auto operator<=>(CAmmoInfo const& lhs, CAmmoInfo const& rhs) noexcept
{
	return _strnicmp(
		lhs.m_szName.data(),
		rhs.m_szName.data(),
		std::max(lhs.m_szName.length(), rhs.m_szName.length())
	) <=> 0;
}
...
export [[nodiscard]] bool operator==(std::string_view lhs, CAmmoInfo const& rhs) noexcept { return (lhs <=> rhs) == std::strong_ordering::equal; }
export [[nodiscard]] bool operator==(CAmmoInfo const& lhs, std::string_view rhs) noexcept { return (lhs <=> rhs) == std::strong_ordering::equal; }
export [[nodiscard]] bool operator==(CAmmoInfo const& lhs, CAmmoInfo const& rhs) noexcept { return (lhs <=> rhs) == std::strong_ordering::equal; }
