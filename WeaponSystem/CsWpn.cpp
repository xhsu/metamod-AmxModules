...
struct G18C : CPrefabWeapon
{
	...
	qboolean UseDecrement(void) noexcept override { return true; }
	...
	void Spawn() noexcept override
	{
		...
		g_engfuncs.pfnSetModel(edict(), "models/w_glock18.mdl");
	...
	void Precache() noexcept override
	{
		g_engfuncs.pfnPrecacheModel("models/v_glock18.mdl");
		g_engfuncs.pfnPrecacheModel("models/w_glock18.mdl");
		g_engfuncs.pfnPrecacheModel("models/shield/v_shield_glock18.mdl");

		g_engfuncs.pfnPrecacheSound("weapons/glock18-1.wav");
		g_engfuncs.pfnPrecacheSound("weapons/glock18-2.wav");
		g_engfuncs.pfnPrecacheSound("weapons/clipout1.wav");
		g_engfuncs.pfnPrecacheSound("weapons/clipin1.wav");
		g_engfuncs.pfnPrecacheSound("weapons/sliderelease1.wav");
		g_engfuncs.pfnPrecacheSound("weapons/slideback1.wav");
		g_engfuncs.pfnPrecacheSound("weapons/357_cock1.wav");
		g_engfuncs.pfnPrecacheSound("weapons/de_clipin.wav");
		g_engfuncs.pfnPrecacheSound("weapons/de_clipout.wav");

		m_iShellId = m_iShell = g_engfuncs.pfnPrecacheModel("models/pshell.mdl");
		m_usFireGlock18 = g_engfuncs.pfnPrecacheEvent(1, "events/glock18.sc");
	}

	int GetItemInfo(ItemInfo* p) noexcept override
	{
		p->pszName = STRING(pev->classname);
		p->pszAmmo1 = "9mm";
		p->iMaxAmmo1 = 120;
	...
	void PrimaryAttack() noexcept override
	{
		if (m_iWeaponState & WPNSTATE_GLOCK18_BURST_MODE)
		{
			...
			{
				GLOCK18Fire(1.2f * (1.f - m_flAccuracy), 0.5f, true);
			}
			else if (m_pPlayer->pev->velocity.Length2D() > 0)
			{
				GLOCK18Fire(0.185f * (1.f - m_flAccuracy), 0.5f, true);
			}
			else if (m_pPlayer->pev->flags & FL_DUCKING)
			{
				GLOCK18Fire(0.095f * (1.f - m_flAccuracy), 0.5f, true);
			}
			else
			{
				GLOCK18Fire(0.3f * (1.f - m_flAccuracy), 0.5f, true);
			}
		}
		else
		{
			if (!(m_pPlayer->pev->flags & FL_ONGROUND))
			{
				GLOCK18Fire(1.f * (1.f - m_flAccuracy), 0.2f, false);
			}
			else if (m_pPlayer->pev->velocity.Length2D() > 0)
			{
				GLOCK18Fire(0.165f * (1.f - m_flAccuracy), 0.2f, false);
			}
			else if (m_pPlayer->pev->flags & FL_DUCKING)
			{
				GLOCK18Fire(0.075f * (1.f - m_flAccuracy), 0.2f, false);
			}
			else
			{
				GLOCK18Fire(0.1f * (1.f - m_flAccuracy), 0.2f, false);
			}
		}
	}
	...
	void GLOCK18Fire(float flSpread, float flCycleTime, bool bFireBurst) noexcept
	{
		if (bFireBurst)
		{
			m_iGlock18ShotsFired = 0;
		...
		auto const vecAiming = gpGlobals->v_forward;

		auto const vecDir = m_pPlayer->FireBullets3(vecSrc, vecAiming, flSpread, 8192, 1, BULLET_PLAYER_9MM, std::lroundf(GLOCK18_DAMAGE), GLOCK18_RANGE_MODIFER, m_pPlayer->pev, true, m_pPlayer->random_seed);

#ifdef CLIENT_WEAPONS
		EFEV const flag = FEV_NOTHOST;
#else
		EFEV const flag{};
#endif

		g_engfuncs.pfnPlaybackEvent(flag, m_pPlayer->edict(), m_usFireGlock18, 0, (float*)&g_vecZero, (float*)&g_vecZero, vecDir.x, vecDir.y,
			int(m_pPlayer->pev->punchangle.pitch * 100), int(m_pPlayer->pev->punchangle.yaw * 100), m_iClip == 0, false);
	...
	void WeaponIdle() noexcept override
	{
		...
		{
			auto flRand = UTIL_Random(0.f, 1.f);
